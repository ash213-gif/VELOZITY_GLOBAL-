...
function TaskBar({
  ...
  const start = task.createdAt ? new Date(task.createdAt) : new Date(task.dueDate);
  const end = new Date(task.dueDate);

  const leftPercent = (differenceInDays(start, monthStart) / totalDays) * 100;
...
export default function TimeLine() {
  ...
  const todayOffset = (differenceInDays(new Date(), monthStart) / totalDays) * 100;
